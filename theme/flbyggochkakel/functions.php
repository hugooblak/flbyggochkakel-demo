<?php
/**
 * FL BYGG & KAKEL AB theme functions.
 *
 * Kept short on purpose. Design lives in theme.json, markup in templates and patterns.
 * Lead capture lives in the "FL BYGG & KAKEL AB Leads" plugin, so leads survive a theme change.
 *
 ...
 * Patterns read these through fl_lead() so one demo can be rebuilt for any firm.
 */
...
add_action(
	'wp_enqueue_scripts',
	function () {
		wp_enqueue_style( 'flbyggochkakel', get_stylesheet_uri(), array(), wp_get_theme()->get( 'Version' ) );
	}
);
add_action(
	'after_setup_theme',
...
add_action(
	'init',
	function () {
		register_block_style( 'core/paragraph', array( 'name' => 'eyebrow', 'label' => __( 'Eyebrow label', 'flbyggochkakel' ) ) );
		register_block_style( 'core/list', array( 'name' => 'checklist', 'label' => __( 'Tick list', 'flbyggochkakel' ) ) );

		register_block_pattern_category( 'flbyggochkakel-sections', array( 'label' => __( 'FL BYGG & KAKEL AB sections', 'flbyggochkakel' ) ) );
		register_block_pattern_category( 'flbyggochkakel-pages', array( 'label' => __( 'FL BYGG & KAKEL AB page layouts', 'flbyggochkakel' ) ) );
	}
);
