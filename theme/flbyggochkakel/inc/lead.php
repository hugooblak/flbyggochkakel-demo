 ...
 * or photo. They call fl_lead() instead. To build a demo for a real firm, the
 ...
 *
 * With no lead-data.php present the defaults below render the fictional FL BYGG & KAKEL AB.
 *
 ...
 * @return string
 */
function fl_sample_note() {
	return fl_lead( 'ar_riktig' ) ? '' : ' <span class="fl-sample">(platshållare)</span>';
}
 ...
 * @return string File name for fl_lead_img().
 */
function fl_lead_bild( $i ) {
	$bilder = (array) fl_lead( 'bilder', array() );
 ...
 * @return string
 */
function fl_ring_label() {
	$namn = fl_lead( 'kontakt' );
	return $namn ? sprintf( 'Ring %s · %s', $namn, fl_lead( 'telefon' ) ) : 'Ring ' . fl_lead( 'telefon' );
}
