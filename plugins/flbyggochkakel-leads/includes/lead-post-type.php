...
add_action(
	...
	function () {
		register_post_type(
			...
			array(
				'labels'          => array(
					'name'          => __( 'Leads', 'flbyggochkakel-leads' ),
					'singular_name' => __( 'Lead', 'flbyggochkakel-leads' ),
					'edit_item'     => __( 'Förfrågan', 'flbyggochkakel-leads' ),
					'search_items'  => __( 'Search leads', 'flbyggochkakel-leads' ),
					'not_found'     => __( 'Inga förfrågningar än. Skicka formuläret så dyker den upp här.', 'flbyggochkakel-leads' ),
					'all_items'     => __( 'Alla förfrågningar', 'flbyggochkakel-leads' ),
				),
...
add_filter(
	...
	function () {
		return array(
			...
			'title'        => __( 'Name', 'flbyggochkakel-leads' ),
			'npl_phone'    => __( 'Phone', 'flbyggochkakel-leads' ),
			'npl_service'  => __( 'Needs', 'flbyggochkakel-leads' ),
			'npl_timeline' => __( 'When', 'flbyggochkakel-leads' ),
			'npl_zip'      => __( 'ZIP', 'flbyggochkakel-leads' ),
			'npl_source'   => __( 'Source', 'flbyggochkakel-leads' ),
			'npl_status'   => __( 'Status', 'flbyggochkakel-leads' ),
			'date'         => __( 'Received', 'flbyggochkakel-leads' ),
		);
	}
);
...
function npl_source_summary( $post_id ) {
	...
	return __( 'Direct', 'flbyggochkakel-leads' );
}
...
add_action(
	...
	function () {
		add_meta_box( 'npl_details', __( 'What they told us', 'flbyggochkakel-leads' ), 'npl_details_box', 'npl_lead', 'normal', 'high' );
		add_meta_box( 'npl_status', __( 'Status', 'flbyggochkakel-leads' ), 'npl_status_box', 'npl_lead', 'side', 'high' );
	}
);
...
function npl_details_box( $post ) {
	$rows = array(
		__( 'Name', 'flbyggochkakel-leads' )     => get_post_meta( $post->ID, 'name', true ),
		__( 'Phone', 'flbyggochkakel-leads' )    => get_post_meta( $post->ID, 'phone', true ),
		__( 'E-post', 'flbyggochkakel-leads' )    => get_post_meta( $post->ID, 'email', true ),
		__( 'ZIP', 'flbyggochkakel-leads' )      => get_post_meta( $post->ID, 'zip', true ),
		__( 'Needs', 'flbyggochkakel-leads' )    => npl_label( 'service', get_post_meta( $post->ID, 'service', true ) ),
		__( 'When', 'flbyggochkakel-leads' )     => npl_label( 'timeline', get_post_meta( $post->ID, 'timeline', true ) ),
		__( 'Source', 'flbyggochkakel-leads' )   => npl_source_summary( $post->ID ),
	);
	...
	$rows[ __( 'Samtycke lämnat', 'flbyggochkakel-leads' ) ] = get_post_meta( $post->ID, 'consent_at', true ) . ' — "' . get_post_meta( $post->ID, 'consent_text', true ) . '"';
...
function npl_status_box( $post ) {
	...
	echo '<label class="screen-reader-text" for="npl-status">' . esc_html__( 'Status', 'flbyggochkakel-leads' ) . '</label><select id="npl-status" name="npl_status" style="width:100%">';
