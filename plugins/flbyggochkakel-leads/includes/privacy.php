...
add_filter(
	...
	function ( $exporters ) {
		$exporters['flbyggochkakel-leads'] = array(
			'exporter_friendly_name' => __( 'Quote requests', 'flbyggochkakel-leads' ),
			'callback'               => function ( $email ) {
				$items = array();
				foreach ( npl_leads_by_email( $email ) as $lead ) {
					$data = array( array( 'name' => __( 'Received', 'flbyggochkakel-leads' ), 'value' => $lead->post_date ) );
					...
					$items[] = array(
						'group_id'    => 'flbyggochkakel-leads',
						'group_label' => __( 'Quote requests', 'flbyggochkakel-leads' ),
...
add_filter(
	...
	function ( $erasers ) {
		$erasers['flbyggochkakel-leads'] = array(
			'eraser_friendly_name' => __( 'Quote requests', 'flbyggochkakel-leads' ),
			'callback'             => function ( $email ) {
				$removed = false;
				foreach ( npl_leads_by_email( $email ) as $lead ) {
